<x-app title="Properties">
    <p class="flex--center-full font-Large  text-success mt-5 max-sm:font-large" > {{session('success')}}</p>
    <div class="flex--center-full my-10 max-sm:flex-col">
        <div class="flex  w-64 h-64  pr-5 m-0 max-sm:pr-0 mb-2 flex-col max-sm:mb-8">
        ...
        <div class="flex  h-full ">
            <form  enctype="multipart/form-data" action="{{route('admin.properties.update',['id' => $properties->id])}}" method="POST"  class="flex flex-col text-center" >
                @csrf
                @method('PATCH')
